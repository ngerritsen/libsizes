...
export function searchLibraries(libraries, searchValue) {
  return libraries.filter(
    library => library.name.toLowerCase().indexOf(searchValue) > -1 || library.used
  );
}
...
export function collapseLibraries(libraries) {
  return groupedLibrariesToArray(groupBy(libraries, library => library.name)).map(libraryVersions =>
    collapseLibrary(libraryVersions)
  );
}

export function sortByVersion(libraryVersions) {
  return libraryVersions.slice().sort((a, b) => semver.lt(a.version, b.version));
}
...
export function resultToMessage(result) {
  ...
  return `Size: ${bytesToKb(normal)}kB, minified: ${bytesToKb(minified)}kB, gzipped ${bytesToKb(
    gzipped
  )}kB.`;
}
